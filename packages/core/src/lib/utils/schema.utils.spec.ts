...
describe('SchemaUtil', () => {
  ...
  it('isPathKeyControl', () => {
    expect(schemaUtil.isPathKeySchema({ kind: 'input', key: 'name' })).toBe(false);
    expect(schemaUtil.isPathKeySchema({ kind: 'input', key: 'name.first' })).toBe(true);
  });
  ...
  it('isAnySchema', () => {
    ...
    expect(schemaUtil.isControl({ kind: 'input' })).toBe(true);
