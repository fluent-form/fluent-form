...
describe('Space compact', () => {
  ...
  it('should be possible to switch compact first / last classes', () => {
    const spaceCompactElement: HTMLElement = fixture.nativeElement;
    const nzInput = spaceCompactElement.querySelector('input[nz-input]');
    const nzInputGroup = spaceCompactElement.querySelector('nz-input-group');
    const nzTreeSelect = spaceCompactElement.querySelector('nz-tree-select');
...
@Component({
  ...
  template: `
    <nz-space-compact [nzSize]="size" [nzBlock]="block">
      @if (showFirst) {
        <input nz-input nzSpaceCompactItem="input" />
      }
      <nz-input-group nzSpaceCompactItem="input"><input nz-input /></nz-input-group>
      <nz-input-number nzSpaceCompactItem="input-number" />
      <nz-date-picker nzSpaceCompactItem="picker" />
      <nz-range-picker nzSpaceCompactItem="picker" />
      <nz-time-picker nzSpaceCompactItem="picker" />
      <nz-cascader nzSpaceCompactItem="select" [nzOptions]="[]" />
      <nz-select nzSpaceCompactItem="select" />
      <nz-tree-select nzSpaceCompactItem="select" [nzNodes]="[]" />
      @if (showLast) {
