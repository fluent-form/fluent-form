...
describe('control-container.builder', () => {
  it('form', () => {
    ...
    const value = { kind: 'group', updateOn: 'blur', schemas: [] } as FormGroupSchema;
    expect(schema).toEqual(value);
  });

  it('group', () => {
    const schema = standardSchema(group('group').schemas());
    const value = { kind: 'group', name: 'group', schemas: [] } as SafeAny;
    expect(schema).toEqual(value);
  });

  it('array', () => {
    const schema = standardSchema(array('array').schemas());
    const value = { kind: 'array', name: 'array', schemas: [] } as SafeAny;
    expect(schema).toEqual(value);
  });
});