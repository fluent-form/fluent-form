...
import { meta } from '../../';
import { story, jsonSource, expressionSource } from './';
...
组件图示的部分入参类型为**布尔值**的参数允许传入一个布尔值，或一个返回布尔值的函数；

> 需要通过 `withStaticExpression()` 开启此特性。

但在 JSON 下是无法编写函数的，这时候可以传入一个字符串类型的静态表达式，该表达式的执行上下文为 `{ model, control, schema }` 。在字符串中可以编写一些简单的 JS 表达式。
...
目前，静态表达式是一个简易的沙箱环境下运行的，可通过一些手段逃逸出沙箱环境；开发者需要确保传入的表达式是安全的，没有被注入的。
